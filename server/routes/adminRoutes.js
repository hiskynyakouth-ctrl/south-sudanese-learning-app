...
const subjectPayload = (s) => ({
  ...
  grade_id: s.grade_id, stream_id: s.stream_id,
  ...
  icon: s.icon || '📘', created_at: s.created_at, updated_at: s.updated_at,
});
...
router.get('/past-papers', async (req, res) => {
  try {
    ...
    res.json(result.rows.map(p => ({
      id: p.id, subject: p.subject||'', grade: p.grade||'', year: p.year,
      paper: p.paper||'', title: p.title||'', url: p.file_url||'', subject_id: p.subject_id||null,
    })));
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.post('/subscriptions/activate', async (req, res) => {
  ...
  const expiry = new Date(Date.now() + (parseInt(days)||60) * 24*60*60*1000);
  try {
    const result = await query(
...
router.delete('/subscriptions/:id', async (req, res) => {
  try {
    await query(`UPDATE users SET subscription_plan='',subscription_expiry=NULL WHERE id=$1`, [req.params.id]);
