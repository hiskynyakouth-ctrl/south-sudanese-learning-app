...
exports.getChaptersBySubject = async (req, res) => {
  try {
    const chapters = await query(
      'SELECT * FROM chapters WHERE subject_id=$1 ORDER BY created_at', [req.params.subjectId]);
    ...
    res.json(chapters.rows.map(ch => ({
      ...
      video_url: ch.video_url, quiz_count: countMap[ch.id] || 0,
    })));
...
exports.getChapter = async (req, res) => {
  try {
    ...
    const questions = await query('SELECT * FROM chapter_questions WHERE chapter_id=$1', [chapter.id]);
    const quiz      = await query('SELECT * FROM quizzes WHERE chapter_id=$1', [chapter.id]);
    res.json({ chapter, questions: questions.rows, quizCount: quiz.rows.length, quizPreview: quiz.rows.slice(0,3) });
  } catch (err) { res.status(500).json({ error: err.message }); }
};
